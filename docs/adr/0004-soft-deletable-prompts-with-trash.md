...
A [Prompt](../CONTEXT.md#prompt) can now be deleted, but deletion is soft: the Prompt gets a `deleted_at` timestamp
rather than being removed, and it — along with all its [Versions](../CONTEXT.md#version) and [Runs](../CONTEXT.md#run) —
disappears from every normal view (lists, detail pages, historical Versions, direct `/runs/:id` links) until it is
restored from [Trash](../CONTEXT.md#trash). We chose soft delete because a hard delete would cascade-destroy Run
history, undermining ADR-0001's premise that a stored Run always points at the precise inputs that produced it. This is
a distinct, coarser operation from ADR-0001's per-Version immutability: individual Versions and Runs remain permanently
undeletable; only a whole Prompt (identity + everything under it) can be deleted, as a unit.
