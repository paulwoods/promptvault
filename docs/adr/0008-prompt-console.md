...
>
> **Amended by [ADR-0014](0014-prompt-write-seam.md).** The decision stands. Its "`PUT` remains the full-save path and keeps serving Create, Duplicate, and the Console's first iteration" consequence has expired — `PUT` is retired, `POST` creates and `PATCH` is the only mutating door. See ADR-0014.
...
- **`PATCH /api/prompts/{id}` exists before it has a caller.** It is committed with tests ahead of the Console consuming it. `PUT` remains the full-save path and keeps serving Create, Duplicate, and the Console's first iteration. *(Expired — [ADR-0014](0014-prompt-write-seam.md) retires `PUT`; Create and Duplicate go through `POST`.)*
