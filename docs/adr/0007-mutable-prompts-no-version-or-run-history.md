...
Saving an edit overwrites what was there. Running a Prompt still streams Claude's response to the browser, but the run
is **not persisted**: no inputs, no rendered prompt, no response, no status, no id. The `version`, `run`, and
`activity_event` tables are dropped, and the Version, Run, and Activity concepts are removed from the glossary entirely.
