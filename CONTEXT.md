...
### Run Settings
The Claude parameters frozen into a Version that make a [Run](#run) reproducible: **model** (one of the supported Claude model IDs), an optional **system prompt**, **max tokens**, and **effort** (`low` / `medium` / `high`). There is deliberately no temperature — the current default models reject it. The system prompt is distinct from the Version's prompt text: the prompt text (with [Variables](#variable) substituted) is sent as the user message, while the system prompt is sent separately.
